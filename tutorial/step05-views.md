...
:green_circle: **Tip:** You'll notice that we're importing the model, form, and tables modules for brevity. If you would prefer to import the relevant classes directly, you're welcome to do so; just remember to change the class definitions below accordingly.
...
The general pattern we'll follow here is to sublass a generic view class provided by NetBox, and define the necessary attributes. We won't need to write any actual logic because the views NetBox provides takes care of all that for us.
...
:green_circle: **Tip:** It occurs to the author that having chosen a model name that ends with "List" might be a bit confusing here. Just remember that `AccessListView` is the _object_ (detail) view, and `AccessListListView` is the _list_ view.

Before we move on to the next view, do you remember the extra column we added to `AccessListTable` in step three? That column expects to find a count of rules assigned for each access list in the queryset, named `rule_count`. Let's add this to our queryset now. We can employ Django's `Count()` function to extend the SQL query and annotate the count. (Don't forget to add the import statement up top.)
...
The rest of our views follow the same exact pattern as the first four.
...
URL mapping for NetBox plugins works pretty much identical to regular Django apps: We'll define `urlpatterns` as an iterable of `path()` calls, mapping URL fragments to view classes.

We'll need to import Django's `path` function from its `urls` module, as well as our plugin's `models` and `views` modules.
...
We've chosen `access-lists` as the base URL for our `AccessList` model, but you are free to choose something different. However, it is recommended to retain the naming scheme shown, as several NetBox features rely on it. Also note that each of the views must be invoked by its `as_view()` method.

:green_circle: **Tip:** The `<int:pk>` string you see in some of the URLs is a [path converter](https://docs.djangoproject.com/en/stable/topics/http/urls/#path-converters). Specifically, this is an integer (`int`) variable named `pk`. This value is passed to the view when the request is processed, so that the desred object can be located in the database.
...
You may recall that one of the features provided by NetBox is automatic change logging. You can see this in action by viewing a NetBox object and selecting the "Changelog" tab. Since our models inherit from `NetBoxModel`, they too can utilize this feature.
...
Notice that we're using `ObjectChangeLogView` directly here; we did not need to create model-specific subclasses for it. Additionally, we're passing an additional keyword argument `model` to the view: This specifies the model to be used (which is why we didn't need to subclass the view).
...
Then, add the `get_absolute_url()` method to the `AccessList` class:
...
`reverse()` takes two arguments here: The view name, and a list of position arguments. The view name is formed by concatenating three components:
...
We'll add a method for `AccessListRule` as well, adjusting the view name accordingly.
...
We see our table has successfully render the `name`, `rule_count`, and `default_action` columns that we defined in step three, and the `default_actions` column shows two rules assigned as expected. And if we click the "Add" button at top right, we'll be taken to the access list creation form.

![Access list creation form](/images/step05-accesslist-form.png)

However, if you click the link to the access list object, you'll be met by a `TemplateDoesNotExist` exception. This means exactly what it says: We have not yet defined a template for this view. Don't worry, that's coming up next!
